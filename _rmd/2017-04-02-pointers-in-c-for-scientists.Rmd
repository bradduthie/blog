...
The above stores the value five in the **Location 2**. On the left hand side of the equaiton, we can read this as 'value at the location pointer_1 plus one'. The parentheses tells C to compute `pointer_1 + 1` before applying the dereference opeartor, so the correct address is identified within parentheses and dereferenced with the asterisk outside of them. We can assign six random values and print them using the function below.
